...
ARG DJANGO_REDIS_URL=redis://redis:6379/0
ARG DJANGO_DATABASE_URL=postgres://user:password@db:5432/app
ARG EMAIL_HOST_USER
...
ENV DJANGO_REDIS_URL=${DJANGO_REDIS_URL} DJANGO_DATABASE_URL=${DJANGO_DATABASE_URL}
...
RUN mkdir -p ${APP_ROOT}/static ${APP_ROOT}/logs && chown -R ${DJANGO_USER}:${DJANGO_USER} ${DJANGO_ROOT} ${APP_ROOT}/static ${APP_ROOT}/logs
...
VOLUME ["${DJANGO_ROOT}", "${APP_ROOT}/logs", "${APP_ROOT}/static"]
